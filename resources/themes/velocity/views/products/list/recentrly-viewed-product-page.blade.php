...
@inject ('productImageHelper', 'Webkul\Product\Helpers\ProductImage')
  ...
  <recently-viewed
      class="fof-recetly-viewed-gorizontal"
...
@push('scripts')
    <script type="text/x-template" id="recently-viewed-template">
        <div :class="`${addClass} recently-viewed`">
            ...
            <div :class="`recetly-viewed-products-wrapper fof-recently-viewed row ${addClassWrapper}`">
                <div
                    :key="Math.random()"
                    class="card grid-card product-card-new col-3"
                    v-for="(product, index) in recentlyViewed">

                    <div class="product-image-container" v-bind:class="{ flappable: product.alterImage }">
                        <a :href="`${baseUrl}/${product.urlKey}`" class="unset">
                            ...
                            <div class="product-image fof-flapp-image-2" :style="`background-image: url(${product.alterImage})`" v-if="product.alterImage"></div>
                        </a>
                    </div>
                    ...
                    <div class="col-12 no-padding card-body align-vertical-top" v-if="product.name" >
                        <a :href="`${baseUrl}/${product.urlKey}`" class="unset no-padding">
                            <div class="product-name">
                                <span class="fs16 text-nowrap" v-html="product.name"></span>
                            </div>
