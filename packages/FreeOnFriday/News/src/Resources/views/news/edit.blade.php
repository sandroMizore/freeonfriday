...
@section('content')
    <div class="content">
        ...
        <form method="POST" action="{{ route('news.update', $post['id']) }}" @submit.prevent="onSubmit" enctype="multipart/form-data">
                ...
                <div class="control-group">
                    <select class="control" id="locale-switcher" name="locale" onChange="window.location.href=this.value">
                        @foreach (core()->getAllLocales() as $localeModel)
                            ...
                            <option
                                value="{{ route('news.edit', $post['id']) . '?locale=' . $localeModel->code }}" {{ ($localeModel->code) == $locale ? 'selected' : '' }}>
                                {{ $localeModel->name }}
                            </option>
                    ...
                    <accordian :title="'{{ __('admin::app.catalog.categories.general') }}'" :active="true">
                        <div slot="body">
                            ...
                            <div class="control-group" :class="[errors.has('name') ? 'has-error' : '']">
                                ...
                                <input type="text" v-validate="'required'" class="control" id="name" name="name" value="{{ $post['title'] }}" data-vv-as="&quot;{{ __('admin::app.catalog.categories.name') }}&quot;" v-slugify-target="'slug'"/>
                    ...
                    <accordian :title="'{{ __('admin::app.catalog.categories.description-and-images') }}'" :active="true">
                        <div slot="body">
                            ...
                            <div class="control-group {!! $errors->has('image.*') ? 'has-error' : '' !!}">
                                ...
                                <image-wrapper :button-label="'{{ __('admin::app.catalog.products.add-image-btn-title') }}'" input-name="image" :multiple="false" :images='@json(url('storage/news/' . $post['image']))'></image-wrapper>
                            ...
                            <div class="control-group">
                                ...
                                <textarea  class="control" id="short_description" name="short_description" />
                                  {{ $post['except'] }}
                                </textarea>
                            </div>
                    ...
                    <accordian :title="'{{ __('admin::app.catalog.categories.seo') }}'" :active="true">
                        <div slot="body">
                            ...
                            <div class="control-group">
                                ...
                                <input type="text" class="control" id="meta_title" name="meta_title" value="{{ $post['meta_title'] }}"/>
                            </div>
                            ...
                            <div class="control-group" :class="[errors.has('slug') ? 'has-error' : '']">
                                ...
                                <input type="text" v-validate="'required'" class="control" id="slug" name="slug" value="{{ $post['slug'] }}" data-vv-as="&quot;{{ __('admin::app.catalog.categories.slug') }}&quot;" v-slugify/>
                            ...
                            <div class="control-group">
                                ...
                                <textarea class="control" id="meta_description" name="meta_description">{{ $post['meta_description'] }}</textarea>
                            </div>

                            <div class="control-group">
                                <label for="meta_keywords">{{ __('admin::app.catalog.categories.meta_keywords') }}</label>
                                <textarea class="control" id="meta_keywords" name="meta_keywords">{{ $post['meta_keywords'] }}</textarea>
                            </div>
